...
def summary(request):
    ...
    # Get study type filters for each map
    map1_study_type = request.GET.get('map1_study_type', 'All')
    map2_study_type = request.GET.get('map2_study_type', 'All')
    
    # Base queryset for variant studies
    ...
    # Get filtered studies for each map
    map1_studies = get_filtered_studies(map1_study_type)
    map2_studies = get_filtered_studies(map2_study_type)

    # Optimized queries remain the same
    ...
    # Get locations for each map independently
    map1_locations = [get_location_data(lat, lon, map1_studies) for lat, lon in location_fields]
    map1_flattened = [item for sublist in map1_locations for item in sublist]
    
    map2_locations = [get_location_data(lat, lon, map2_studies) for lat, lon in location_fields]
    map2_flattened = [item for sublist in map2_locations for item in sublist]

    # Process coordinates for map 1
    map1_counts = defaultdict(int)
    for record in map1_flattened:
        coordinates = (record["latitude"], record["longitude"])
        map1_counts[coordinates] += 1

    # First map (markers)
    m = folium.Map(location=[-4.0335, 21.7501], zoom_start=3)
    for coordinates, value in map1_counts.items():
        try:
    ...
    # Process coordinates for map 2
    map2_counts = defaultdict(int)
    for record in map2_flattened:
        coordinates = (record["latitude"], record["longitude"])
        map2_counts[coordinates] += 1

    # Second map (choropleth)
    ...
    for (lat, lon), value in map2_counts.items():
        try:
    ...
    context = {
        ...
        'map1_locations': map1_flattened,
        'map2_locations': map2_flattened,
        'map': m,
        'map2': map_html,
        'publications_per_country': dict(publications_per_country),
        'map1_study_type': map1_study_type,
        'map2_study_type': map2_study_type,
